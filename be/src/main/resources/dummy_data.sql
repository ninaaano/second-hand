    ...
    begin
           ...
           while i < 1000 do
                  insert into product (product_id, title, contents, price, created_at, modified_at, views, status, deleted, category_id, location_id, user_id)
                      values (i, concat('테스트제목', i), concat('테스트내용', i), floor(rand()*1000000), now(), now(), floor(rand()*100), 'SALE', false, floor(rand()*18) + 1, floor(rand()*22) + 1, floor(rand()*3)+1);
           set i = i + 1;
           end while;
    end $$
delimiter ;
...
create procedure make_chatroom()
       begin
              ...
              while product_idx < 1000 do
                      while chatroom_cnt < chatroom_max do
...
create procedure make_product_image()
       begin
              ...
              while product_idx < 1000 do
                     while image_cnt < image_max do
                            insert into product_image (product_image_id, image_url, product_id)
                            values (image_idx, 'https://second-hand-4.s3.ap-northeast-2.amazonaws.com/product-images/cute_cat.jpeg', product_idx);
